...
const Menu = ({ user, showFilter, onToggleFilter, onLogout, onDownload }) => {
  ...
  const handleItem = (cb) => { cb(); setIsMenuOpen(false); };
          ...
          <View style={[s.overlay, { backgroundColor: c.overlay }]}>
            <TouchableWithoutFeedback>
              <View style={[s.drawer, { backgroundColor: c.surface }]}>
                <SafeAreaView>
                  <View style={[s.drawerHeader, { borderBottomColor: c.border }]}>
                    <Text style={[s.userName, { color: c.text }]} numberOfLines={1}>{user}</Text>
                    <TouchableOpacity onPress={() => setIsMenuOpen(false)} style={s.closeBtn}>
                      <Icons.X size={20} color={c.text} />
                    </TouchableOpacity>
                  </View>

                  <View style={s.section}>
                    <Text style={[s.sectionTitle, { color: c.textMuted }]}>Theme</Text>
                  ...
                  <View style={s.section}>
                    <Text style={[s.sectionTitle, { color: c.textMuted }]}>Actions</Text>
                    <TouchableOpacity style={s.menuItem} onPress={() => handleItem(onDownload)}>
                      <Icons.Download size={20} color={c.primary} />
                      <Text style={[s.menuItemText, { color: c.text }]}>Download Tasks</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={s.menuItem} onPress={() => handleItem(onToggleFilter)}>
                      {showFilter ? <Icons.X size={20} color={c.primary} /> : <Icons.Filter size={20} color={c.primary} />}
                      <Text style={[s.menuItemText, { color: c.text }]}>
                        {showFilter ? 'Hide Filter' : 'Show Filter'}
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={s.menuItem} onPress={() => handleItem(onLogout)}>
