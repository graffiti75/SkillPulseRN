...
const DateTimePicker = ({ isOpen, onClose, onSelect, title }) => {
  ...
  const {
    selectedDate, currentMonth, hours, minutes, step,
    setSelectedDate, setHours, setMinutes, setStep, changeMonth, handleConfirm,
  } = useDateTimePicker();
          ...
          <TouchableWithoutFeedback>
            <View style={[s.sheet, { backgroundColor: c.surface }]}>
              ...
              <View style={[s.header, { borderBottomColor: c.border }]}>
                <TouchableOpacity
                  onPress={() => step === 'time' ? setStep('date') : onClose()}
                  style={s.backBtn}
                >
                  <Icons.ArrowLeft size={20} color={c.text} />
                </TouchableOpacity>
                <Text style={[s.title, { color: c.text }]}>{title || 'Select Date & Time'}</Text>
              </View>

              {/* Tabs */}
              <View style={[s.tabs, { borderBottomColor: c.border }]}>
                <TouchableOpacity
                  style={[s.tab, step === 'date' && [s.tabActive, { borderBottomColor: c.primary }]]}
                  onPress={() => setStep('date')}
                >
                  <Text style={[s.tabText, { color: step === 'date' ? c.primary : c.textMuted }]}>📅 Date</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[s.tab, step === 'time' && [s.tabActive, { borderBottomColor: c.primary }]]}
                  onPress={() => step === 'date' && selectedDate && setStep('time')}
                >
                  <Text style={[s.tabText, { color: step === 'time' ? c.primary : c.textMuted }]}>🕐 Time</Text>
                </TouchableOpacity>
              </View>
              ...
              >
                <Text style={s.confirmText}>{step === 'date' ? 'Next →' : 'Confirm'}</Text>
              </TouchableOpacity>
            </View>
          </TouchableWithoutFeedback>
        </View>
...
const makeStyles = (c) => StyleSheet.create({
  ...
  body: { padding: 16 },
  confirmBtn: {
    margin: 16, height: 50, borderRadius: 14,
