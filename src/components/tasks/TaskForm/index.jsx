...
const TaskForm = ({ isOpen, onClose, onSave, editTask, allTasks = [] }) => {
  ...
  const filteredSuggestions = extractSuggestions(
    allTasks,
    state.description,
    5,
  ).filter((s) => s.toLowerCase() !== state.description.toLowerCase());
      ...
      >
        ...
        <View style={s.formGroup}>
          ...
          <TextInput
            ...
            onFocus={() =>
              state.setShowSuggestions(state.description.length > 0)
            }
            onBlur={() =>
              setTimeout(() => state.setShowSuggestions(false), 200)
            }
            multiline
            numberOfLines={3}
          />
          {state.showSuggestions && filteredSuggestions.length > 0 && (
            ...
            >
              {filteredSuggestions.map((suggestion, i) => (
                ...
                >
                  <Text style={[s.suggestionText, { color: c.text }]}>
                    {suggestion}
                  </Text>
                </TouchableOpacity>
              ))}
        ...
        <View style={s.formGroup}>
          ...
          >
            <Text
              style={[s.inputText, !state.startTime && { color: c.textMuted }]}
            >
              {state.startTime
        ...
        <View style={s.formGroup}>
          ...
          >
            <Text
              style={[s.inputText, !state.endTime && { color: c.textMuted }]}
            >
              {state.endTime
        ...
        <TouchableOpacity
          style={[
            ...
            (!isFormValid || state.isLoading) && s.btnDisabled,
          ]}
          onPress={handleSave}
