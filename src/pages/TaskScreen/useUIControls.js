...
export const useUIControls = () => {
  ...
  const fetchTasks = useCallback(async () => {
    if (!user?.email) { setIsLoading(false); return { success: false, error: 'Not authenticated' }; }
    setIsLoading(true);
  ...
  const loadMore = useCallback(async () => {
    if (!user?.email || showLoadingSpinner || !canLoadMore) return { success: false };
  ...
  const addNewTask = useCallback(async (data) => {
    ...
    const result = await addTask(user.email, data.description, data.startTime, data.endTime);
    if (result.success) {
  ...
  const editTask = useCallback(async (id, data) => {
    if (!user?.email) return { success: false, error: 'Not authenticated' };
    const result = await updateTask(user.email, id, data.description, data.startTime, data.endTime);
    if (result.success) {
      const upd = { id, ...data };
  ...
  return {
    tasks, allTasks, descriptions, isLoading, showLoadingSpinner,
    canLoadMore, filterDate,
    fetchTasks, loadMore, filterByDate, addNewTask, editTask, removeTask,
  };
};
