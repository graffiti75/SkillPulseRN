...
const DownloadScreen = ({ isOpen, onClose }) => {
  ...
  const handleDownload = async () => {
    ...
    if (result.success)
      setAlert({
        message: `Exported ${result.taskCount} tasks`,
        type: "success",
      });
    else
      setAlert({
        message: result.error || "Failed to download tasks",
        type: "error",
      });
  };
            ...
            {["json", "csv"].map((f) => (
              <TouchableOpacity
                ...
                style={[
                  s.formatOption,
                  {
                    borderColor: format === f ? c.primary : c.border,
                    backgroundColor:
                      format === f ? c.primaryLight : c.surfaceAlt,
                  },
                ]}
                onPress={() => setFormat(f)}
              >
                <Text
                  style={[
                    s.formatLabel,
                    { color: format === f ? c.primary : c.text },
                  ]}
                >
                  {f === "json" ? "{ } JSON" : "📊 CSV"}
                </Text>
              ...
              <View
                style={[
                  s.checkbox,
                  {
                    ...
                    backgroundColor: formattedDates ? c.primary : "transparent",
                  },
                ]}
              >
                {formattedDates && <Text style={s.checkmark}>✓</Text>}
              </View>
              <Text style={[s.checkLabel, { color: c.text }]}>
